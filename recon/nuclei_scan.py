...
def build_target_urls(hostnames: Set[str], ips: Set[str], nmap_data: Optional[dict] = None) -> List[str]:
    """
    Build list of target URLs for nuclei scanning.
    Uses nmap data if available to determine open ports/protocols.
    
    Args:
        hostnames: Set of hostnames to scan
        ips: Set of IPs to scan (if NUCLEI_SCAN_ALL_IPS is True)
        nmap_data: Optional nmap scan results for port information
    ...
    # Determine ports from nmap data or use defaults
    http_ports = {80, 8080, 8000, 8888}
    https_ports = {443, 8443, 4443, 9443}
    
    if nmap_data:
        # Extract open HTTP/HTTPS ports from nmap results
        for target_data in list(nmap_data.get("by_ip", {}).values()) + list(nmap_data.get("by_hostname", {}).values()):
            for port_info in target_data.get("ports_scripts", {}).get("tcp", []):
                port = port_info.get("port")
                service = port_info.get("service", "").lower()
                product = port_info.get("product", "").lower()
                
                if port:
                    if "https" in service or "ssl" in service or "tls" in product:
                        https_ports.add(port)
                    elif "http" in service or "web" in service:
                        http_ports.add(port)
    
    # Generate URLs for hostnames
    for hostname in sorted(hostnames):
        # Always add standard HTTP/HTTPS
        urls.append(f"http://{hostname}")
        urls.append(f"https://{hostname}")
        
        # Add non-standard ports if found
        for port in sorted(http_ports - {80}):
            urls.append(f"http://{hostname}:{port}")
        for port in sorted(https_ports - {443}):
            urls.append(f"https://{hostname}:{port}")
...
def run_nuclei_scan(recon_data: dict, output_file: Path = None) -> dict:
    """
    ...
    # Build target URLs using nmap data if available
    nmap_data = recon_data.get("nmap")
    target_urls = build_target_urls(hostnames, ips, nmap_data)
    ...
    if NUCLEI_DAST_MODE:
        ...
        # Get base URLs for crawling (just the main hostnames)
        base_urls = []
        for hostname in sorted(hostnames):
            base_urls.append(f"http://{hostname}")
            base_urls.append(f"https://{hostname}")
        
        # Run Katana to discover URLs with parameters
        dast_urls = run_katana_crawler(base_urls, use_proxy)
    ...
    try:
        ...
        # Add to recon data
        recon_data["nuclei"] = nuclei_results
...
def enrich_recon_file(recon_file: Path) -> dict:
    ...
    # Run nuclei scan
    enriched_data = run_nuclei_scan(recon_data, output_file=recon_file)
